...
class LossAggregator(nn.Module):
    ...
    def _build_loss_(self, loss_cfg):
        ...
        # 从 losses 包中获取名为 loss_cfg['type'] 的 Loss 类 
    ...
    def forward(self, training_feats):
        ...
        # 输入 training_feats 是一个 features dict
        # "k" 是 loss 名, "v" 是 feature 和 label
        for k, v in training_feats.items():
            if k in self.losses:
                ...
                loss = loss.mean() * loss_func.loss_term_weight # 乘以 loss 项的权重
                loss_sum += loss
            # 如果 "k" 不在已建 losses 中，且 "v" 是 torch.Tensor，那么计算它的 loss 并添加到 loss_sum
            else:
